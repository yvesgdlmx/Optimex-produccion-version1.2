...
const JobCompleteHora = () => {
    ...
    useEffect(() => {
        const obtenerMeta = async () => {
            ...
            const metaJobComplete = data.registros.find(registro => registro.name === '32 JOB COMPLETE');
            if (metaJobComplete) {
                setMeta(metaJobComplete.meta);
            }
        };
        obtenerMeta();
    }, []);
    ...
    useEffect(() => {
        const obtenerRegistros = async () => {
            ...
            const registrosJobComplete = data.registros.filter(registro => registro.name.includes('JOB COMPLETE'));
            // Filtrar los registros que están entre las 06:30 y las 23:00
            const registrosFiltrados = registrosJobComplete.filter(registro => {
    ...
    return (
        <>
            <div className="generado-hora" id="jobcomplete">
                <table className="tabla">
                    ...
                    <tbody>
                        <tr className="tabla__tr">
                            <Link to={'/jobcomplete-horas'} className="link__tabla">
                                <div className="tabla__th-flex">
                                    <img src="./img/ver.png" alt="imagen-ver" className="tabla__ver" />
                                    <td className="tabla__td position">Producción <br/> <span className="tabla__td-span">Meta: <span className="tabla__span-meta">{meta}</span></span></td>
                                </div>
                            </Link>
...
export default JobCompleteHora;