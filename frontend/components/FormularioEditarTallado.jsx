...
const FormularioEditarTallado = ({ meta }) => {
    ...
    const editarMeta = async () => {
        try {
            ...
            const { data } = await clienteAxios.put(`/metas/metas-tallados/editar/${meta.id}`, metaActualizada);
            mostrarAlerta({
                msg: 'Meta actualizada correctamente',
                error: false
            });
            setTimeout(() => {
                navigate('/meta-tallado');
            }, 1200);
        } catch (error) {
            console.error(error);
            mostrarAlerta({
...
export default FormularioEditarTallado;