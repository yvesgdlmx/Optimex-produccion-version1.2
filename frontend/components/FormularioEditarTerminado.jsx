...
const FormularioEditarTerminado = ({ meta }) => {
    ...
    const editarMeta = async () => {
        try {
            ...
            const { data } = await clienteAxios.put(`/metas/metas-terminados/editar/${meta.id}`, metaActualizada);
            mostrarAlerta({
                msg: 'Meta actualizada correctamente',
                error: false
            });
            setTimeout(() => {
                navigate('/meta-terminado');
            }, 1200);
        } catch (error) {
            console.error(error);
            mostrarAlerta({
...
export default FormularioEditarTerminado;